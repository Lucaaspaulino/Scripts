    ...
    for device in devices:
        try:
            with ConnectHandler(**device) as net_connect:
                ...
                port_10g = net_connect.send_command('show port description | match FREE | match 10G | count', expect_string=escaped_prompt).strip()
                print(f'Saída do comando para 10G do dispositivo {device["host"]}: {port_10g}')
                time.sleep(3)  # Pequena pausa para garantir que o comando foi processado

                port_100g = net_connect.send_command('show port description | match FREE | match 100G | count', expect_string=escaped_prompt).strip()
